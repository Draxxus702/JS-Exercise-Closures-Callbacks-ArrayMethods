...
function processContains(item, list, callback) {
    const yes = list.filter(checkedItem => checkedItem === item);
...
function getRunnersByTShirtSize(runners, tShirtSize) {
    const newShirt = runners.filter((runner) => {
...
function counterMakerWithLimit(maxValue) {
    ...
    function counter() {
        if (count <= maxValue) { return count++ } else if (count == maxValue + 1) {
            count = 0
            return count++
        }
    }
